...
**Panel structure (reordered 2026-08-19):** the daily-essentials view is always fully visible, in this order — Bodyweight (the motivator, first thing seen), Daily Intake, 7-Day Snapshot (line/area chart, % of daily target). Everything else (Recovery, Bloodwork, Today's Log, Debrief, Advisory Board) lives in collapsed `<details>` sections below, open on tap. Speed over rigor is the current priority — keep turnaround fast, estimate with caveats rather than slow down to double-check every sync.
