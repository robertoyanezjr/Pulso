...
**Panel structure (as of 2026-08-18):** the daily-essentials view is always fully visible — 7-Day Snapshot, Bodyweight, Today's Intake. Everything else (Recovery, Today's Log, Debrief, Trend Telemetry, Advisory Board) lives in collapsed `<details>` sections below, open on tap — full detail stays one tap away, but doesn't compete with the essentials for scroll length. Speed over rigor is the current priority — keep turnaround fast, estimate with caveats rather than slow down to double-check every sync.
