...
**Panel structure (as of 2026-08-18):** the daily-essentials view is always fully visible — 7-Day Snapshot (now a line/area chart, % of daily target, not a table), Bodyweight, Today's Intake. Everything else (Recovery, Today's Log, Debrief, Advisory Board) lives in collapsed `<details>` sections below, open on tap. Speed over rigor is the current priority — keep turnaround fast, estimate with caveats rather than slow down to double-check every sync.
